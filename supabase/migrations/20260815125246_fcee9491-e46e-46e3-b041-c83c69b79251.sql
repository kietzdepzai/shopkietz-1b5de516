...
BEGIN
  ...
  v_pkg_count := jsonb_array_length(coalesce(v_product.boost_packages, '[]'::jsonb));
  IF v_pkg_count > 0 THEN
    IF p_package_index IS NULL OR p_package_index < 0 OR p_package_index >= v_pkg_count THEN
    ...
    v_price := coalesce((v_pkg ->> 'price')::integer, v_product.price);
  END IF;
  ...
  RETURN jsonb_build_object('success', true, 'order_code', v_order_code, 'order_id', v_order_id, 'package_name', v_pkg_name, 'price', v_price);
END; $function$;