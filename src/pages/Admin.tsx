    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={`bg-card border-r border-border flex flex-col transition-all duration-300 ${sidebarOpen ? "w-64" : "w-16"}`}>
        <div className="p-4 border-b border-border flex items-center gap-2">
          ...
          {sidebarOpen && (
            <span className="font-display text-sm font-bold text-primary neon-text tracking-wider">ADMIN</span>
          )}
          <button
        ...
        <div className="p-2 border-t border-border space-y-1">
          ...
          <button
            onClick={signOut}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-destructive hover:bg-muted transition-all"
          >
